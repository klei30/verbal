...
The outgoing verbal request body is encapsulated in one of the following formats:
...
It is important to note that exchanged messages must be Stringified JSONs where verbal will send its messages using the [Request message](connect) format
and the server must send its messages using the [`Response`](#Response) format. Example messages: <br />
verbal message: <br />
`'{"messages":[{"role":"user","text":"Message from verbal"}]}'` <br />
Server message: <br />
...
`signals` is a map of functions which are used to notify verbal on the status of the request and its result. The available
...
chatElementRef.request = {
  ...
  handler: (_, signals) => {
    try {
      ...
      websocket.onerror = () => {
        // 'Connection error' is a special string that will also display in verbal
...
Object containing functions that are used to notify the verbal component about the status of the current request.
The `stopClicked` and `newUserMessage` functions are triggered by verbal itself and contain `listener` properties
